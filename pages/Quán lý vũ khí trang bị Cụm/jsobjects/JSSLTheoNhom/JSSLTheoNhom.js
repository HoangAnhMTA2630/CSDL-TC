export default {
  async fetchData() {
    let success = false;
    let time_delay = Math.random() * 1000;
    let retryCount = 0;
    const maxRetries = 5;
    const delay = ms => new Promise(resolve => setTimeout(resolve, ms));

    while (!success && retryCount < maxRetries) {
      try {
        await SLTheoNhom.run(); // Đổi tên hàm theo truy vấn của bạn
        if (SLTheoNhom.data && SLTheoNhom.data.length > 0) {
          success = true;
        } else {
          throw new Error("Không có dữ liệu");
        }
      } catch (error) {
        retryCount++;
        console.log(`Thử lại lần ${retryCount}: ${error.message}`);
        await delay(time_delay);
      }
    }

    if (!success) {
      throw new Error("Không thể lấy dữ liệu sau nhiều lần thử.");
    }

    const dataRows = SLTheoNhom.data.map(row => ({
      nhom: row["nhom"] ?? "Không rõ",
      count: Number(row["so_luong_thiet_bi"])
    }));

    const nhoms = dataRows.map(item => item.nhom);
    const counts = dataRows.map(item => item.count);

    const options = {
      tooltip: {
        trigger: "axis",
        axisPointer: {
          type: "shadow"
        },
        formatter: "{b}: {c} thiết bị"
      },
      title: {
        text: "Số lượng thiết bị theo nhóm",
        left: "center"
      },
      grid: {
        left: 15,
        right: 15,
        bottom: 30,
        top: 100,
        containLabel: true
      },
      xAxis: [
        {
          type: "category",
          data: nhoms,
          name: "Nhóm"
        }
      ],
      yAxis: [
        {
          type: "value",
          name: "Số lượng",
          minInterval: 1
        }
      ],
      series: [
        {
          name: "Thiết bị",
          type: "bar",
          data: counts,
          itemStyle: {
            color: "#91CC75"
          }
        }
      ]
    };

    return options;
  }
};
