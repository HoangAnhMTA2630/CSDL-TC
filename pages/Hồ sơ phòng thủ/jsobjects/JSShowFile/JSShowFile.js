export default {
	ShowDDF: async () => {
  const path = data_tableHSPT.triggeredRow.duong_dan_file;
  if (!path) {
  ...
  ShowSDTC: async () => {
    const path = data_tableHSPT.triggeredRow.so_do_tuyen_cap;
    if (!path) {
      showAlert("Không có đường dẫn file!", "error");
      return null;
    }
	...
	ShowSDLG: async () => {
    const path = data_tableHSPT.triggeredRow.so_do_logic;
    if (!path) {
      showAlert("Không có đường dẫn file!", "error");
      return null;
    }
	...
	ShowSTR: async () => {
    const path = data_tableHSPT.triggeredRow.so_do_rack;
    if (!path) {
      showAlert("Không có đường dẫn file!", "error");
      return null;
    }
